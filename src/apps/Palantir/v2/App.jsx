// Palantír 2.0 shell (Session 4): thin frame, Files surface only, read-only preview at /palantir2.
...
function Shell(){
  ...
  const {live,lastSync,reload,loading}=useStore();
  ...
  return(
    <div style={{[SCALE_VAR]:scale,fontFamily:T.font,display:'flex',flexDirection:'column',height:'100vh',background:T.bg,overflow:'hidden',color:T.tx}}>
      ...
      <div style={{background:T.hdr,borderBottom:`1px solid ${T.bd}`,padding:'0 12px',display:'flex',alignItems:'center',height:44,flexShrink:0}}>
        ...
        <span style={{fontSize:sc(9),fontWeight:700,color:T.y,background:'rgba(212,146,42,0.12)',borderRadius:8,padding:'1px 6px',marginRight:12,flexShrink:0}}>v2 · read-only</span>
        ...
        <div style={{marginLeft:'auto',display:'flex',alignItems:'center',gap:10,flexShrink:0}}>
          <span style={{display:'inline-flex',alignItems:'center',gap:5,fontSize:sc(10),color:T.tx3}}>
            <span style={{width:6,height:6,borderRadius:'50%',background:live?T.g:T.tx3,display:'inline-block'}}/>
            {live?'live':'offline'}{synced?(' · '+synced):''}
          </span>
