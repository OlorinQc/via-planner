...
const DELIVERABLE_TYPES=[
  ...
  {value:'qa',label:'Q&A'},
  {value:'message_map',label:'Message Map'},
  {value:'briefing_note',label:'Briefing Note'},
  {value:'internal_comms',label:'Internal Communication'},
  {value:'employee_comms',label:'Employee Communication'},
  {value:'passenger_comms',label:'Passenger Communication'},
  {value:'stakeholder_comms',label:'Stakeholder Note'},
  {value:'social_content',label:'Social Content'},
  {value:'speech',label:'Speech'},
  {value:'board_document',label:'Board Document'},
  {value:'website_publication',label:'Website Publication'},
  {value:'newswire',label:'Newswire Release'},
...
// ─── FLEXIBLE DATES ───────────────────────────────────────────────────────────
// FlexDate: { precision: 'exact'|'range'|'week'|'month'|'tbd', date, startDate, endDate, year, month, weekStartDate, label, confidence: 'confirmed'|'tentative'|'unknown' }
const FLEX_PRECISION=[
...
const FLEX_CONFIDENCE=[
  {value:'confirmed',label:'Confirmed'},
  {value:'tentative',label:'Tentative'},
  {value:'unknown',  label:'Unknown'},
];

function mkFlexDate(precision='exact',vals={}){
  return {precision,date:null,startDate:null,endDate:null,year:null,month:null,weekStartDate:null,label:'',confidence:'tentative',...vals};
}

function fmtFlex(fd){
  if(!fd) return null;
  if(typeof fd==='string') return fmt(fd); // legacy string date
  ...
  if(precision==='exact'&&date) return conf+fmt(date);
  if(precision==='range'&&startDate&&endDate) return conf+fmt(startDate)+' – '+fmt(endDate);
  if(precision==='week'&&weekStartDate){const d=pd(weekStartDate);return conf+'Week of '+MONTHS_SHORT[d.getMonth()]+' '+d.getDate();}
  if(precision==='month'&&year&&month!=null) return conf+MONTHS_SHORT[month-1]+' '+year;
  return label||'—';
}

function flexToExact(fd){
  // Returns a YYYY-MM-DD string for calendar bucketing or null
  if(!fd) return null;
  if(typeof fd==='string') return fd;
  if(fd.precision==='exact') return fd.date;
  if(fd.precision==='range') return fd.startDate;
  if(fd.precision==='week') return fd.weekStartDate;
  return null; // month/tbd — no exact date
}

function dsFromFlex(fd){
  const s=flexToExact(fd);
  return s?ds(s):null;
}

function flexDueColor(fd){
  const st=dsFromFlex(fd);
  return st?DD[st]:null;
}

// ─── BUILT-IN TEMPLATES ───────────────────────────────────────────────────────
// offsetDays: days before the target date (negative = before, 0 = on date, positive = after)
...
const BUILT_IN_TEMPLATES=[
  {
    id:'tpl-foundational',name:'Foundational Phase',
    description:'The common starting phase for all communications files: mandate, source material, key messages, validation.',
    deliverableType:'foundational_phase',
    defaultDurationDays:14,
    taskTemplates:FOUNDATIONAL_TASKS,
  },
  {
    id:'tpl-press-release',name:'Press Release',
    description:'Full press release production cycle including foundational phase, drafting, quote approvals, and publication.',
    deliverableType:'press_release',
    defaultDurationDays:21,
    includesFoundational:true,
    taskTemplates:[
      ...FOUNDATIONAL_TASKS,
      {title:'Draft press release',notes:'Prepare the first draft using approved key messages and confirmed facts.',offsetDays:-7,requiresApproval:false},
      {title:'Secure quote approvals',notes:'Confirm internal and external quotes, names, titles, and bilingual alignment where needed.',offsetDays:-4,requiresApproval:false},
      {title:'Secure final content approval',notes:'Coordinate approval by file lead, director, executive, and external partner where required.',offsetDays:-3,requiresApproval:true},
      {title:'Prepare website publication',notes:'Prepare online version for publication, including title, body, metadata, and final approved content.',offsetDays:-2,requiresApproval:false},
      {title:'Prepare newswire version',notes:'Prepare newswire-ready version, confirm timing, distribution settings, and final approval.',offsetDays:-1,requiresApproval:false},
      {title:'Publish',notes:'Coordinate final publication through website and newswire channels.',offsetDays:0,requiresApproval:false},
      {title:'Monitor pickup and follow-up questions',notes:'Track media pickup, stakeholder reaction, follow-up questions, and required responses.',offsetDays:1,requiresApproval:false},
    ],
  },
  {
    id:'tpl-media-statement',name:'Media Statement',
    description:'Reactive or proactive media statement: draft, validate, approve, finalize.',
    deliverableType:'media_statement',
    defaultDurationDays:7,
    taskTemplates:[
      {title:'Draft media statement',notes:'Prepare draft using approved key messages and confirmed facts.',offsetDays:-5,requiresApproval:false},
      {title:'Validate facts and positioning',notes:'Confirm accuracy of all claims, facts, and positioning with business owner.',offsetDays:-3,requiresApproval:false},
      {title:'Secure approval',notes:'Coordinate approval by file lead and appropriate executive.',offsetDays:-2,requiresApproval:true},
      {title:'Finalize and share approved version',notes:'Prepare final version for distribution. Add to SharePoint.',offsetDays:0,requiresApproval:false},
    ],
  },
  {
    id:'tpl-qa',name:'Q&A',
    description:'Question and answer document: draft, validate, approve, finalize.',
    deliverableType:'qa',
    defaultDurationDays:10,
    taskTemplates:[
      {title:'Draft Q&A',notes:'Prepare anticipated questions and draft answers using approved key messages.',offsetDays:-7,requiresApproval:false},
      {title:'Validate facts and positioning',notes:'Verify accuracy of all answers with business owner and subject matter experts.',offsetDays:-4,requiresApproval:false},
      {title:'Secure approval',notes:'Coordinate approval by file lead and appropriate stakeholders.',offsetDays:-2,requiresApproval:true},
      {title:'Finalize and link SharePoint version',notes:'Prepare final version and add SharePoint link to the deliverable.',offsetDays:0,requiresApproval:false},
    ],
  },
  {
    id:'tpl-internal-comms',name:'Internal Communication',
    description:'Internal email or article: draft, approve, distribute.',
    deliverableType:'internal_comms',
    defaultDurationDays:7,
    taskTemplates:[
      {title:'Draft internal communication',notes:'Prepare draft using approved key messages. Confirm audience and channel.',offsetDays:-5,requiresApproval:false},
      {title:'Approve internal communication',notes:'Coordinate approval with appropriate leadership.',offsetDays:-3,requiresApproval:true},
      {title:'Prepare publication or distribution',notes:'Finalize distribution list, format, and timing.',offsetDays:-1,requiresApproval:false},
      {title:'Publish or send',notes:'Distribute through confirmed channel.',offsetDays:0,requiresApproval:false},
    ],
  },
  {
    id:'tpl-employee-comms',name:'Employee Communication',
    description:'Employee-facing communication: draft, approve, distribute.',
    deliverableType:'employee_comms',
    defaultDurationDays:7,
    taskTemplates:[
      {title:'Draft employee communication',notes:'Prepare draft for employee audience using approved messaging.',offsetDays:-5,requiresApproval:false},
      {title:'Approve employee communication',notes:'Coordinate approval with HR and appropriate leadership.',offsetDays:-3,requiresApproval:true},
      {title:'Prepare distribution',notes:'Confirm channel, distribution list, timing, and bilingual requirements.',offsetDays:-1,requiresApproval:false},
      {title:'Send or publish',notes:'Distribute through confirmed channel.',offsetDays:0,requiresApproval:false},
    ],
  },
  {
    id:'tpl-passenger-comms',name:'Passenger Communication',
    description:'Passenger-facing communication: draft, approve, coordinate, publish.',
    deliverableType:'passenger_comms',
    defaultDurationDays:7,
    taskTemplates:[
      {title:'Draft passenger communication',notes:'Prepare draft for passenger audience. Confirm language and channel.',offsetDays:-5,requiresApproval:false},
      {title:'Approve passenger communication',notes:'Coordinate approval with Operations and appropriate leadership.',offsetDays:-3,requiresApproval:true},
      {title:'Coordinate channel publication',notes:'Confirm timing with web, digital, and station teams.',offsetDays:-1,requiresApproval:false},
      {title:'Publish or send',notes:'Execute final publication across confirmed channels.',offsetDays:0,requiresApproval:false},
    ],
  },
  {
    id:'tpl-stakeholder-comms',name:'Stakeholder Note',
    description:'Stakeholder note or briefing: draft, approve, send.',
    deliverableType:'stakeholder_comms',
    defaultDurationDays:7,
    taskTemplates:[
      {title:'Draft stakeholder note',notes:'Prepare note for target stakeholder audience.',offsetDays:-4,requiresApproval:false},
      {title:'Approve stakeholder note',notes:'Coordinate approval with file lead and appropriate executive.',offsetDays:-2,requiresApproval:true},
      {title:'Prepare distribution list',notes:'Confirm recipient list and any personalization required.',offsetDays:-1,requiresApproval:false},
      {title:'Send note',notes:'Distribute to stakeholders.',offsetDays:0,requiresApproval:false},
    ],
  },
  {
    id:'tpl-social',name:'Social Content',
    description:'Social media content: draft, approve, schedule/publish.',
    deliverableType:'social_content',
    defaultDurationDays:5,
    taskTemplates:[
      {title:'Draft social copy',notes:'Prepare social content aligned with key messages. Confirm platforms and formats.',offsetDays:-3,requiresApproval:false},
      {title:'Approve social copy',notes:'Coordinate approval with appropriate lead and confirm imagery.',offsetDays:-2,requiresApproval:true},
      {title:'Schedule or publish social copy',notes:'Schedule or publish on confirmed platforms and timing.',offsetDays:0,requiresApproval:false},
    ],
  },
  {
    id:'tpl-message-map',name:'Message Map',
    description:'Core messaging framework for a file or initiative.',
    deliverableType:'message_map',
    defaultDurationDays:10,
    taskTemplates:[
      {title:'Draft key messages',notes:'Draft core message, supporting messages, proof points, and anticipated opposition.',offsetDays:-7,requiresApproval:false},
      {title:'Validate key messages',notes:'Review with business owner and subject matter experts.',offsetDays:-4,requiresApproval:false},
      {title:'Secure approval',notes:'Confirm final message map with appropriate leadership.',offsetDays:-2,requiresApproval:true},
      {title:'Finalize and add to SharePoint',notes:'Store final approved version in SharePoint and link to file.',offsetDays:0,requiresApproval:false},
    ],
  },
];
...
// ─── PRIMITIVES ───────────────────────────────────────────────────────────────
const Chip=({text,bg,tx,small})=>(<span style={{fontSize:small?9:10,fontWeight:600,padding:small?'1px 5px':'2px 8px',borderRadius:10,background:bg,color:tx,whiteSpace:'nowrap',display:'inline-block',fontFamily:T.font}}>{text}</span>);
const StatusDot=({map,val})=>{const c=map?.[val];if(!c)return null;return<span style={{display:'inline-flex',alignItems:'center',gap:4,fontSize:10,fontWeight:600,padding:'2px 7px',borderRadius:10,background:c.bg,color:c.tx}}><span style={{width:5,height:5,borderRadius:'50%',background:c.tx,display:'inline-block',flexShrink:0}}/>{c.label}</span>;};
...
function FlexDateInput({value,onChange,label}){
  ...
  return(
    <div>
      {label&&<span style={ss.lbl}>{label}</span>}
      <div style={{display:'flex',gap:4,alignItems:'center',flexWrap:'wrap'}}>
        <select value={precision} onChange={e=>{const p=e.target.value;onChange(mkFlexDate(p,{confidence}));}} style={{...ss.sel,width:'auto',flex:'0 0 110px',fontSize:11}}>
          {FLEX_PRECISION.map(p=><option key={p.value} value={p.value}>{p.label}</option>)}
        </select>
        {precision==='exact'&&<input type="date" value={fd?.date||''} onChange={e=>upd({date:e.target.value||null})} style={inputStyle}/>}
        {precision==='range'&&<><input type="date" value={fd?.startDate||''} onChange={e=>upd({startDate:e.target.value||null})} style={inputStyle}/><span style={{color:T.tx3,fontSize:11}}>to</span><input type="date" value={fd?.endDate||''} onChange={e=>upd({endDate:e.target.value||null})} style={inputStyle}/></>}
        {precision==='week'&&<input type="date" value={fd?.weekStartDate||''} onChange={e=>upd({weekStartDate:e.target.value||null})} style={inputStyle} title="Select the Monday of the week"/>}
        {precision==='month'&&<><select value={fd?.month||1} onChange={e=>upd({month:parseInt(e.target.value)})} style={{...ss.sel,width:'auto',fontSize:11}}>{MONTHS_SHORT.map((m,i)=><option key={i} value={i+1}>{m}</option>)}</select><input type="number" value={fd?.year||new Date().getFullYear()} onChange={e=>upd({year:parseInt(e.target.value)||null})} style={{...inputStyle,width:70}} min={2024} max={2030}/></>}
        {precision==='tbd'&&<input value={fd?.label||''} onChange={e=>upd({label:e.target.value})} placeholder="Optional note…" style={inputStyle}/>}
        <select value={confidence} onChange={e=>upd({confidence:e.target.value})} style={{...ss.sel,width:'auto',fontSize:11,flex:'0 0 100px'}}>
          {FLEX_CONFIDENCE.map(c=><option key={c.value} value={c.value}>{c.label}</option>)}
        </select>
      </div>
    </div>
  );
  ...
  return(
    <div style={{width:360,flexShrink:0,borderLeft:`1px solid ${T.bd}`,background:T.s1,overflowY:'auto',maxHeight:'100%'}}>
      <div style={{padding:'12px 14px',borderBottom:`1px solid ${T.bd}`,display:'flex',alignItems:'flex-start',gap:8,position:'sticky',top:0,background:T.s1,zIndex:5}}>
        <div style={{flex:1}}>
          {file&&<div style={{fontSize:9,fontWeight:700,color:T.tx3,textTransform:'uppercase',marginBottom:1,letterSpacing:'0.5px'}}>{file.title}</div>}
          {dv&&<div style={{fontSize:9,color:T.acc,marginBottom:2}}>↳ {dv.title}</div>}
          <textarea value={task.title} onChange={e=>upd({title:e.target.value})} rows={2} style={{width:'100%',border:'none',outline:'none',background:'transparent',fontSize:13,fontWeight:600,color:T.tx,resize:'none',lineHeight:1.4,fontFamily:T.font}}/>
        </div>
      ...
      <div style={{padding:'12px 14px'}}>
        ...
        <Fld label="Assignees">
          <div style={{display:'flex',flexWrap:'wrap',gap:3,marginBottom:4}}>{taskAssignees(task).map(a=>(<span key={a} style={{fontSize:10,padding:'2px 6px',borderRadius:10,background:'rgba(91,156,246,0.12)',color:T.acc,display:'flex',alignItems:'center',gap:3}}>{a}<button onClick={()=>upd({assignees:taskAssignees(task).filter(x=>x!==a)})} style={{background:'transparent',border:'none',cursor:'pointer',color:T.acc,padding:0,fontSize:10,lineHeight:1}}>×</button></span>))}</div>
          <select value="" onChange={e=>{if(e.target.value&&!taskAssignees(task).includes(e.target.value))upd({assignees:[...taskAssignees(task),e.target.value]});}} style={ss.sel}><option value="">+ Add assignee</option>{allPeople.filter(m=>!taskAssignees(task).includes(m)).map(m=><option key={m}>{m}</option>)}</select>
        </Fld>
        <Fld label="Dependencies">
          {(task.dependsOn||[]).map(did=>{const dep=data.tasks.find(t=>t.id===did);return dep?(<div key={did} style={{display:'flex',alignItems:'center',gap:6,padding:'4px 0',borderBottom:`1px solid ${T.bd3}`}}><span style={{fontSize:9,padding:'1px 5px',borderRadius:10,background:isDone(dep)?'rgba(63,182,139,0.12)':'rgba(217,95,95,0.12)',color:isDone(dep)?T.g:T.r}}>{isDone(dep)?'✓':'⏳'}</span>{onOpenTask?<button onClick={()=>onOpenTask(did)} style={{flex:1,background:'transparent',border:'none',cursor:'pointer',fontSize:11,color:T.acc,textAlign:'left',padding:0,textDecoration:'underline'}}>{dep.title}</button>:<span style={{flex:1,fontSize:11,color:T.tx2}}>{dep.title}</span>}<button onClick={()=>upd({dependsOn:(task.dependsOn||[]).filter(x=>x!==did)})} style={{background:'transparent',border:'none',cursor:'pointer',color:T.tx3,fontSize:13}}>×</button></div>):null;})}
          <select value="" onChange={e=>{if(e.target.value&&!(task.dependsOn||[]).includes(e.target.value))upd({dependsOn:[...(task.dependsOn||[]),e.target.value]});}} style={{...ss.sel,marginTop:4}}><option value="">+ Add dependency</option>{fileTasks.filter(t=>!(task.dependsOn||[]).includes(t.id)).map(t=><option key={t.id} value={t.id}>{t.title}</option>)}</select>
        </Fld>
...
function DeliverablePanel({dvId,data,onClose,saveDeliverable,delDeliverable,saveTask,delTask,newTask}){
  ...
  const dvTasks=data.tasks.filter(t=>t.deliverableId===dvId||((t.fileId||t.projectId)===dv.fileId&&!t.deliverableId&&false)); // only linked tasks
  const openTasks=dvTasks.filter(t=>!isDone(t));
    ...
    <div style={{width:400,flexShrink:0,borderLeft:`1px solid ${T.bd}`,background:T.s1,overflowY:'auto',maxHeight:'100%',display:'flex',flexDirection:'column'}}>
      <div style={{padding:'12px 14px',borderBottom:`1px solid ${T.bd}`,position:'sticky',top:0,background:T.s1,zIndex:5}}>
        <div style={{display:'flex',alignItems:'flex-start',gap:8}}>
          <div style={{flex:1}}>
            {file&&<div style={{fontSize:9,fontWeight:700,color:T.tx3,textTransform:'uppercase',marginBottom:2,letterSpacing:'0.5px'}}>{file.title}</div>}
        ...
        <div style={{display:'flex',gap:4,flexWrap:'wrap',marginTop:4}}>
          ...
          {dv.type&&<Chip text={dvLabel(dv.type)} bg={T.s3} tx={T.acc2} small/>}
        </div>
      </div>
      <div style={{padding:'12px 14px',flex:1,overflowY:'auto'}}>
        ...
        <Fld label="Owner">
          <select value={dv.ownerName||''} onChange={e=>upd({ownerName:e.target.value})} style={ss.sel}><option value="">—</option>{allPeople.map(m=><option key={m}>{m}</option>)}</select>
        </Fld>
        ...
        <Fld label="Approval Status">
          <select value={dv.approvalStatus||'not_required'} onChange={e=>upd({approvalStatus:e.target.value})} style={ss.sel}><option value="not_required">Not required</option><option value="pending">Pending</option><option value="partially_approved">Partially approved</option><option value="approved">Approved</option><option value="changes_requested">Changes requested</option></select>
        </Fld>
        {dv.sharePointUrl&&<Fld label="SharePoint Link"><a href={dv.sharePointUrl} target="_blank" rel="noreferrer" style={{fontSize:12,color:T.acc}}>Open document →</a></Fld>}
        {!dv.sharePointUrl&&<Fld label="SharePoint Link"><Inp value={dv.sharePointUrl||''} onChange={v=>upd({sharePointUrl:v})} placeholder="https://viarailonline.sharepoint.com/…"/></Fld>}
        ...
        <div style={{borderTop:`1px solid ${T.bd}`,paddingTop:10,marginTop:4}}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:6}}>
            <span style={ss.lbl}>TASKS ({openTasks.length} open)</span>
            <button onClick={()=>setAddingTask(true)} style={{...ss.btn,fontSize:10}}>+ Add task</button>
          </div>
          ...
          {dvTasks.map(t=>(
            <div key={t.id} onClick={()=>setSelTask(selTask===t.id?null:t.id)} style={{display:'flex',alignItems:'center',gap:6,padding:'6px 0',borderBottom:`1px solid ${T.bd3}`,cursor:'pointer'}} onMouseEnter={e=>e.currentTarget.style.background=T.s2} onMouseLeave={e=>e.currentTarget.style.background='transparent'}>
              <div style={{flex:1,minWidth:0}}>
                <div style={{fontSize:11,color:isDone(t)?T.tx3:T.tx,textDecoration:isDone(t)?'line-through':'none',lineHeight:1.3}}>{t.title}</div>
                <div style={{display:'flex',gap:3,marginTop:2}}><StatusDot map={TS} val={t.status}/>{t.dueDate&&<DueChip date={t.dueDate}/>}</div>
              </div>
              {!isDone(t)&&<button onClick={e=>{e.stopPropagation();saveTask(t.id,{status:'completed'});}} style={{...ss.btn,fontSize:9,padding:'2px 6px',color:T.g,borderColor:'rgba(63,182,139,0.25)'}}>✓</button>}
            </div>
          ))}
...
function ApplyTemplateModal({file,data,onClose,onApply}){
  const [step,setStep]=useState(1); // 1=pick template, 2=configure, 3=preview
  const [selTpl,setSelTpl]=useState(null);
  const [targetDate,setTargetDate]=useState(mkFlexDate('exact',{confidence:'tentative'}));
  const [ownerName,setOwnerName]=useState(data.people?.find(p=>p.name==='Karl')?.name||'Karl');
  ...
  const buildPreview=()=>{
    ...
    const tasks=tpl.taskTemplates
      .filter(tt=>skipFoundational?!FOUNDATIONAL_TASKS.find(ft=>ft.title===tt.title):true)
      .map((tt,i)=>({
        id:`preview-${i}`,title:tt.title,notes:tt.notes,
        dueDate:exactDate?addDays(exactDate,tt.offsetDays):null,
        status:'not_started',assignees:[ownerName],
  ...
  const handleApply=()=>{
    ...
    const dvId=uid();
    const deliverable={
      id:dvId,fileId:file.id,
      title:tpl.name+(file.title?` — ${file.title}`:''),
      type:tpl.deliverableType==='foundational_phase'?'other':tpl.deliverableType,
      ownerName,supportNames:[],
      status:'not_started',priority:'medium',
      dueDate:targetDate,publicationDate:null,
      approvalStatus:'not_required',approverNames:[],
      taskIds:[],sharePointUrl:'',templateId:tpl.id,notes:'',
      createdAt:TODAY_STR,updatedAt:TODAY_STR,
    };
    const tasks=tpl.taskTemplates
      .filter(tt=>skipFoundational?!FOUNDATIONAL_TASKS.find(ft=>ft.title===tt.title):true)
      .map(tt=>({
        id:uid(),title:tt.title,notes:tt.notes||'',
        fileId:file.id,projectId:file.id,deliverableId:dvId,
        dueDate:exactDate?addDays(exactDate,tt.offsetDays):null,
        status:'not_started',assignees:[ownerName],
        dependsOn:[],dependencies:[],gate:'',link:null,approvalChain:[],
        source:'template',templateId:tpl.id,createdAt:TODAY_STR,
      }));
    deliverable.taskIds=tasks.map(t=>t.id);
    onApply(deliverable,tasks);
    onClose();
  };
  ...
  return(
    <Overlay onClose={onClose} wide>
      ...
      {step===1&&(
        <div>
          <p style={{fontSize:12,color:T.tx2,margin:'0 0 14px'}}>Select a template to generate a deliverable and its tasks.</p>
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:6}}>
            {BUILT_IN_TEMPLATES.map(t=>(
              <div key={t.id} onClick={()=>setSelTpl(t.id)} style={{padding:'10px 12px',border:`1.5px solid ${selTpl===t.id?T.acc:T.bd}`,borderRadius:7,cursor:'pointer',background:selTpl===t.id?T.s3:T.s2}} onMouseEnter={e=>{if(selTpl!==t.id)e.currentTarget.style.borderColor=T.bd2;}} onMouseLeave={e=>{if(selTpl!==t.id)e.currentTarget.style.borderColor=T.bd;}}>
                <div style={{fontSize:12,fontWeight:600,color:T.tx,marginBottom:2}}>{t.name}</div>
                <div style={{fontSize:10,color:T.tx3,lineHeight:1.4}}>{t.description}</div>
                <div style={{fontSize:9,color:T.tx3,marginTop:4}}>{t.taskTemplates.length} tasks</div>
              </div>
            ))}
          </div>
          <div style={{marginTop:14,display:'flex',justifyContent:'flex-end'}}>
            <button onClick={()=>{if(selTpl)setStep(2);}} disabled={!selTpl} style={{...ss.btnP,opacity:selTpl?1:0.4}}>Configure →</button>
          </div>
        </div>
      )}
      {step===2&&tpl&&(
        <div>
          <div style={{marginBottom:12,padding:'8px 10px',background:T.s2,borderRadius:5,border:`1px solid ${T.bd}`}}>
            <div style={{fontSize:11,fontWeight:600,color:T.tx,marginBottom:2}}>{tpl.name}</div>
            <div style={{fontSize:10,color:T.tx3}}>{tpl.taskTemplates.length} tasks · {tpl.defaultDurationDays} day default timeline</div>
          </div>
          <Fld label="Target date (due date or publication date)">
            <FlexDateInput value={targetDate} onChange={setTargetDate}/>
          </Fld>
          <Fld label="Deliverable owner">
            <select value={ownerName} onChange={e=>setOwnerName(e.target.value)} style={ss.sel}>{allPeople.map(m=><option key={m}>{m}</option>)}</select>
          </Fld>
          {tpl.includesFoundational&&(
            <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:10}}>
              <input type="checkbox" id="skipf" checked={skipFoundational} onChange={e=>setSkipFoundational(e.target.checked)}/>
              <label htmlFor="skipf" style={{fontSize:12,color:T.tx2,cursor:'pointer'}}>Skip foundational phase tasks (mandate, source material, key messages) — already done for this file</label>
            </div>
          )}
          <div style={{display:'flex',gap:6,justifyContent:'space-between',marginTop:4}}>
            <button onClick={()=>setStep(1)} style={ss.btn}>← Back</button>
            <button onClick={buildPreview} style={ss.btnP}>Preview tasks →</button>
          </div>
        </div>
      )}
      {step===3&&preview&&(
        <div>
          <div style={{marginBottom:10}}>
            <div style={{fontSize:11,fontWeight:600,color:T.tx,marginBottom:4}}>Tasks to be created ({preview.length})</div>
            <div style={{fontSize:10,color:T.tx3,marginBottom:8}}>Dates are calculated backward from your target date. You can edit them after creating.</div>
          </div>
          <div style={{border:`1px solid ${T.bd}`,borderRadius:6,overflow:'hidden',marginBottom:12}}>
            {preview.map((t,i)=>(
              <div key={i} style={{display:'flex',alignItems:'center',gap:8,padding:'7px 10px',borderBottom:`1px solid ${T.bd3}`,background:i%2===0?T.s1:T.s2}}>
                <div style={{flex:1}}>
                  <div style={{fontSize:11,color:T.tx,fontWeight:500}}>{t.title}</div>
                  {t.notes&&<div style={{fontSize:10,color:T.tx3,marginTop:1}}>{t.notes.slice(0,80)}{t.notes.length>80?'…':''}</div>}
                </div>
                <div style={{display:'flex',gap:4,flexShrink:0}}>
                  {t.dueDate?<DueChip date={t.dueDate}/>:<span style={{fontSize:9,color:T.tx3}}>No date</span>}
          ...
          <div style={{display:'flex',gap:6,justifyContent:'space-between'}}>
            <button onClick={()=>setStep(2)} style={ss.btn}>← Back</button>
            <button onClick={handleApply} style={ss.btnP}>✓ Create deliverable & tasks</button>
          </div>
        </div>
      )}
    </Overlay>
...
function FileCard({file,data,onClick,selected}){
  ...
  return(
    <div onClick={onClick} style={{background:selected?T.s3:T.s1,border:`1px solid ${selected?T.acc:T.bd}`,borderRadius:8,padding:'12px 14px',cursor:'pointer',transition:'border-color .15s, background .15s',boxShadow:selected?`0 0 0 1px ${T.acc}40`:'none'}} onMouseEnter={e=>{if(!selected){e.currentTarget.style.borderColor=T.bd2;e.currentTarget.style.background=T.s2;}}} onMouseLeave={e=>{if(!selected){e.currentTarget.style.borderColor=T.bd;e.currentTarget.style.background=T.s1;}}}>
      ...
      {nextTask&&<div style={{fontSize:11,color:T.tx2,background:T.s2,borderRadius:4,padding:'4px 8px',marginBottom:6,borderLeft:`2px solid ${T.acc}40`}}>
        <span style={{color:T.tx3,fontSize:9,fontWeight:700,textTransform:'uppercase',letterSpacing:'0.4px'}}>Next · </span>{nextTask.title.slice(0,50)}
        {nextTask.dueDate&&<span style={{float:'right'}}><DueChip date={nextTask.dueDate}/></span>}
      </div>}
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
        <div style={{display:'flex',gap:6}}>
          <span style={{fontSize:10,color:T.tx3}}>{openTasks.length} task{openTasks.length!==1?'s':''}</span>
          {openDVs.length>0&&<span style={{fontSize:10,color:T.acc}}>{openDVs.length} deliverable{openDVs.length!==1?'s':''}</span>}
        </div>
...
function FilePage({file,data,onClose,saveFile,saveTask,delTask,newTask,addLogEntry,saveDeliverable,delDeliverable,newDeliverable,applyTemplate}){
  ...
  const TABS=[
    ...
    {id:'deliverables',label:`Deliverables (${openDVs.length})`},
  ...
  const tabBtn=id=>({padding:'6px 12px',fontSize:11,fontWeight:600,border:'none',cursor:'pointer',color:tab===id?T.acc:T.tx3,borderBottom:`2px solid ${tab===id?T.acc:'transparent'}`,background:'transparent',transition:'color .1s',fontFamily:T.font});
    ...
    <div style={{display:'flex',flexDirection:'column',height:'100%',overflow:'hidden'}}>
      ...
      <div style={{padding:'14px 16px',borderBottom:`1px solid ${T.bd}`,background:T.hdr,flexShrink:0}}>
        <div style={{display:'flex',alignItems:'flex-start',gap:10,marginBottom:10}}>
          <div style={{flex:1}}>
            {editingTitle?<input autoFocus value={titleVal} onChange={e=>setTitleVal(e.target.value)} onBlur={()=>{saveFile(file.id,{title:titleVal});setEditingTitle(false);}} onKeyDown={e=>{if(e.key==='Enter'){saveFile(file.id,{title:titleVal});setEditingTitle(false);}if(e.key==='Escape')setEditingTitle(false);}} style={{...ss.inp,fontSize:16,fontWeight:700,background:'transparent',borderColor:T.acc}}/>:<h2 onClick={()=>setEditingTitle(true)} style={{fontSize:16,fontWeight:700,color:T.tx,margin:0,cursor:'text',lineHeight:1.2}}>{file.title}</h2>}
          </div>
      ...
      {/* Tabs */}
      <div style={{display:'flex',gap:0,borderBottom:`1px solid ${T.bd}`,background:T.s1,flexShrink:0,paddingLeft:6,overflowX:'auto'}}>
      ...
      <div style={{flex:1,overflow:'hidden',display:'flex'}}>
        <div style={{flex:1,overflowY:'auto',padding:'14px 16px'}}>
          ...
          {tab==='memory'&&(
            <div>
              ...
              {file.latestUpdate&&<div style={{marginTop:10,padding:'8px 10px',background:T.s2,borderRadius:5,borderLeft:`2px solid ${T.acc}50`}}><span style={{fontSize:9,fontWeight:700,color:T.tx3,textTransform:'uppercase',display:'block',marginBottom:2}}>Latest update</span><span style={{fontSize:12,color:T.tx2}}>{file.latestUpdate}</span></div>}
              {addingLog&&(
                <div style={{marginTop:14,border:`1px solid ${T.bd2}`,borderRadius:6,padding:'12px'}}>
                  ...
                  <div style={{display:'flex',gap:4}}><button onClick={()=>{if(logText.trim()){addLogEntry(file.id,logText,logTitle||'Update');setLogText('');setLogTitle('');setAddingLog(false);}}} style={ss.btnP}>Add entry</button><button onClick={()=>{setAddingLog(false);setLogText('');setLogTitle('');}} style={ss.btn}>Cancel</button></div>
                </div>
              )}
          ...
          {tab==='deliverables'&&(
            <div>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:10}}>
                <span style={{fontSize:11,color:T.tx2}}>{openDVs.length} open · {fileDeliverables.filter(isDoneDV).length} done</span>
                <div style={{display:'flex',gap:4}}>
                  <button onClick={()=>setShowTemplateModal(true)} style={{...ss.btn,fontSize:10,color:T.acc,borderColor:`rgba(91,156,246,0.3)`}}>From template</button>
                  <button onClick={()=>setAddingDv(true)} style={ss.btnP}>+ Add deliverable</button>
                </div>
              </div>
              {addingDv&&(
                <div style={{border:`1px solid ${T.bd2}`,borderRadius:6,padding:'12px',marginBottom:12}}>
                  <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,marginBottom:8}}>
                    ...
                    <Fld label="Type" mb={0}><select value={newDvForm.type} onChange={e=>setNewDvForm(x=>({...x,type:e.target.value}))} style={ss.sel}>{DELIVERABLE_TYPES.map(t=><option key={t.value} value={t.value}>{t.label}</option>)}</select></Fld>
                    <Fld label="Owner" mb={0}><select value={newDvForm.ownerName} onChange={e=>setNewDvForm(x=>({...x,ownerName:e.target.value}))} style={ss.sel}><option value="">—</option>{allPeople.map(m=><option key={m}>{m}</option>)}</select></Fld>
                  ...
                  <div style={{display:'flex',gap:4}}><button onClick={()=>{if(newDvForm.title.trim()){newDeliverable({...newDvForm,fileId:file.id,taskIds:[],sharePointUrl:'',notes:'',approvalStatus:'not_required',approverNames:[],supportNames:[],dueDate:null,publicationDate:null,createdAt:TODAY_STR,updatedAt:TODAY_STR});setNewDvForm({title:'',type:'press_release',ownerName:'Karl',status:'not_started'});setAddingDv(false);}}} style={ss.btnP}>Add</button><button onClick={()=>setAddingDv(false)} style={ss.btn}>Cancel</button></div>
                </div>
              )}
              {fileDeliverables.length===0&&<div style={{fontSize:12,color:T.tx3,fontStyle:'italic',padding:'20px 0',textAlign:'center'}}>No deliverables yet. Add one manually or use a template.</div>}
              {fileDeliverables.filter(d=>!isDoneDV(d)).map(dv=>{
                const dvTasks=data.tasks.filter(t=>t.deliverableId===dv.id);
                const openT=dvTasks.filter(t=>!isDone(t)).length;
                const dvs=DVS[dv.status]||DVS.not_started;
                return(
                  <div key={dv.id} onClick={()=>setSelDv(selDv===dv.id?null:dv.id)} style={{display:'flex',alignItems:'center',gap:10,padding:'10px 12px',border:`1px solid ${selDv===dv.id?T.acc:T.bd}`,borderRadius:6,marginBottom:5,cursor:'pointer',background:selDv===dv.id?T.s3:T.s2,transition:'all .1s'}} onMouseEnter={e=>{if(selDv!==dv.id)e.currentTarget.style.borderColor=T.bd2;}} onMouseLeave={e=>{if(selDv!==dv.id)e.currentTarget.style.borderColor=T.bd;}}>
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{fontSize:12,fontWeight:600,color:T.tx,marginBottom:4}}>{dv.title}</div>
                    ...
                    <div style={{textAlign:'right',flexShrink:0}}>
                      <div style={{fontSize:10,color:T.tx3}}>{openT} task{openT!==1?'s':''}</div>
                    </div>
                  </div>
                );
              })}
              {fileDeliverables.filter(isDoneDV).length>0&&(
                <details style={{marginTop:8}}>
                  <summary style={{fontSize:10,color:T.tx3,cursor:'pointer',padding:'4px 0'}}>Completed deliverables ({fileDeliverables.filter(isDoneDV).length})</summary>
                  {fileDeliverables.filter(isDoneDV).map(dv=>(
                    <div key={dv.id} style={{display:'flex',alignItems:'center',gap:6,padding:'6px 10px',borderBottom:`1px solid ${T.bd3}`,opacity:0.5}}>
                      <span style={{fontSize:11,color:T.tx3,flex:1}}>{dv.title}</span>
                      <StatusDot map={DVS} val={dv.status}/>
                    </div>
                  ))}
                </details>
          ...
          {tab==='tasks'&&(
            <div>
              ...
              {addingTask&&<div style={{display:'flex',gap:4,marginBottom:10}}><input autoFocus value={newTaskTitle} onChange={e=>setNTT(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'&&newTaskTitle.trim()){newTask({title:newTaskTitle.trim(),fileId:file.id,projectId:file.id,assignees:['Karl'],status:'not_started',dueDate:null,dependsOn:[],dependencies:[],gate:'',notes:'',link:null,approvalChain:[],source:'manual',createdAt:TODAY_STR});setNTT('');setAddingTask(false);}if(e.key==='Escape')setAddingTask(false);}} placeholder="Task title…" style={{...ss.inp,flex:1}}/><button onClick={()=>setAddingTask(false)} style={ss.btn}>Cancel</button></div>}
              {/* Group by deliverable */}
              {(data.deliverables||[]).filter(d=>d.fileId===file.id).map(dv=>{
                const dvTasks=fileTasks.filter(t=>t.deliverableId===dv.id&&!isDone(t));
                if(!dvTasks.length) return null;
                return(
                  <div key={dv.id} style={{marginBottom:10}}>
                    <div style={{fontSize:9,fontWeight:700,color:T.acc,textTransform:'uppercase',letterSpacing:'0.5px',marginBottom:4,paddingLeft:2}}>↳ {dv.title}</div>
                    {dvTasks.map(task=><TaskRow key={task.id} task={task} data={data} selTask={selTask} setSelTask={setSelTask} saveTask={saveTask}/>)}
                  </div>
                );
              })}
              {/* Ungrouped tasks */}
              {fileTasks.filter(t=>!isDone(t)&&!t.deliverableId).length>0&&(
                <div style={{marginBottom:10}}>
                  {(data.deliverables||[]).filter(d=>d.fileId===file.id).length>0&&<div style={{fontSize:9,fontWeight:700,color:T.tx3,textTransform:'uppercase',letterSpacing:'0.5px',marginBottom:4,paddingLeft:2}}>File tasks</div>}
                  {fileTasks.filter(t=>!isDone(t)&&!t.deliverableId).map(task=><TaskRow key={task.id} task={task} data={data} selTask={selTask} setSelTask={setSelTask} saveTask={saveTask}/>)}
                </div>
              )}
              {fileTasks.filter(isDone).length>0&&<details style={{marginTop:8}}><summary style={{fontSize:10,color:T.tx3,cursor:'pointer',padding:'4px 0'}}>Completed ({fileTasks.filter(isDone).length})</summary>{fileTasks.filter(isDone).map(task=><div key={task.id} style={{display:'flex',alignItems:'center',gap:8,padding:'5px 8px',borderBottom:`1px solid ${T.bd3}`,opacity:0.5}}><span style={{fontSize:11,color:T.tx3,flex:1,textDecoration:'line-through'}}>{task.title}</span><StatusDot map={TS} val={task.status}/></div>)}</details>}
            ...
            <div>
              ...
              {addingMilestone&&(
                <div style={{border:`1px solid ${T.bd2}`,borderRadius:6,padding:'12px',marginBottom:12}}>
                  ...
                  <Fld label="Status" mb={8}><select value={newMilestone.status} onChange={e=>setNM(x=>({...x,status:e.target.value}))} style={ss.sel}>{MILESTONE_STATUS.map(s=><option key={s} value={s}>{MS_LABEL[s]}</option>)}</select></Fld>
                  <div style={{display:'flex',gap:4}}><button onClick={()=>{if(newMilestone.title.trim()){saveFile(file.id,{milestones:[...(file.milestones||[]),{id:uid(),...newMilestone}]});setNM({title:'',status:'not_started',date:''});setAddingMilestone(false);}}} style={ss.btnP}>Add</button><button onClick={()=>setAddingMilestone(false)} style={ss.btn}>Cancel</button></div>
                </div>
              )}
              ...
              {(file.milestones||[]).map(m=>(
                <div key={m.id} style={{display:'flex',alignItems:'center',gap:8,padding:'8px 10px',borderBottom:`1px solid ${T.bd3}`}}>
                  ...
                  <select value={m.status} onChange={e=>saveFile(file.id,{milestones:(file.milestones||[]).map(x=>x.id===m.id?{...x,status:e.target.value}:x)})} style={{...ss.sel,width:'auto',fontSize:10,padding:'2px 6px'}}>{MILESTONE_STATUS.map(s=><option key={s} value={s}>{MS_LABEL[s]}</option>)}</select>
            ...
            <div>
              ...
              {addingLink&&(
                <div style={{border:`1px solid ${T.bd2}`,borderRadius:6,padding:'12px',marginBottom:12}}>
                  <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,marginBottom:8}}>
                    ...
                    <Fld label="Type" mb={0}><select value={newLink.type} onChange={e=>setNL(x=>({...x,type:e.target.value}))} style={ss.sel}>{LINK_TYPES.map(t=><option key={t} value={t}>{t.replace(/_/g,' ')}</option>)}</select></Fld>
                  </div>
                  <Fld label="URL" mb={8}><Inp value={newLink.url} onChange={v=>setNL(x=>({...x,url:v}))} placeholder="https://viarailonline.sharepoint.com/…"/></Fld>
                  <div style={{display:'flex',gap:4}}><button onClick={()=>{if(newLink.url.trim()){saveFile(file.id,{sharePointLinks:[...(file.sharePointLinks||[]),{id:uid(),createdAt:TODAY_STR,...newLink}]});setNL({label:'',url:'',type:'folder'});setAddingLink(false);}}} style={ss.btnP}>Add</button><button onClick={()=>setAddingLink(false)} style={ss.btn}>Cancel</button></div>
                </div>
              )}
              {(file.sharePointLinks||[]).length===0&&<div style={{fontSize:12,color:T.tx3,fontStyle:'italic'}}>No links yet.</div>}
              {(file.sharePointLinks||[]).map(lnk=>(
                <div key={lnk.id} style={{display:'flex',alignItems:'center',gap:8,padding:'8px 10px',border:`1px solid ${T.bd}`,borderRadius:5,marginBottom:5,background:T.s2}}>
                  <div style={{flex:1,minWidth:0}}><a href={lnk.url} target="_blank" rel="noreferrer" style={{fontSize:12,color:T.acc,fontWeight:500,display:'block',marginBottom:2}}>{lnk.label||lnk.url}</a><span style={{fontSize:9,color:T.tx3,textTransform:'uppercase',letterSpacing:'0.4px'}}>{(lnk.type||'folder').replace(/_/g,' ')}</span></div>
            ...
            <div>
              ...
              {addingLog&&(
                <div style={{border:`1px solid ${T.bd2}`,borderRadius:6,padding:'12px',marginBottom:12}}>
                  ...
                  <Fld label="Summary" mb={6}><Inp value={logText} onChange={setLogText} placeholder="Describe what changed and who confirmed it…" rows={3}/></Fld>
                  <div style={{display:'flex',gap:4}}><button onClick={()=>{if(logText.trim()){addLogEntry(file.id,logText,logTitle||'Update');setLogText('');setLogTitle('');setAddingLog(false);}}} style={ss.btnP}>Add</button><button onClick={()=>{setAddingLog(false);setLogText('');setLogTitle('');}} style={ss.btn}>Cancel</button></div>
                </div>
              )}
        ...
        {selDv&&tab==='deliverables'&&(
          <DeliverablePanel dvId={selDv} data={data} onClose={()=>setSelDv(null)} saveDeliverable={saveDeliverable} delDeliverable={delDeliverable} saveTask={saveTask} delTask={delTask} newTask={newTask}/>
        )}
      </div>
...
// Shared task row component used inside FilePage tasks tab
function TaskRow({task,data,selTask,setSelTask,saveTask}){
  const blocked=isBlocked(task,data.tasks);
  return(
    <div onClick={()=>setSelTask(selTask===task.id?null:task.id)} style={{display:'flex',alignItems:'center',gap:8,padding:'7px 10px',borderBottom:`1px solid ${T.bd3}`,cursor:'pointer',background:selTask===task.id?T.s3:'transparent',borderRadius:4}} onMouseEnter={e=>{if(selTask!==task.id)e.currentTarget.style.background=T.s2;}} onMouseLeave={e=>{if(selTask!==task.id)e.currentTarget.style.background='transparent';}}>
      <div style={{flex:1,minWidth:0}}>
      ...
      {!isDone(task)&&<button onClick={e=>{e.stopPropagation();saveTask(task.id,{status:'completed'});}} style={{...ss.btn,fontSize:9,padding:'2px 6px',color:T.g,borderColor:'rgba(63,182,139,0.25)'}}>✓</button>}
    </div>
  );
}
...
function Dashboard({data,saveFile,saveTask,delTask,newTask,addLogEntry,saveDeliverable,delDeliverable,newDeliverable,applyTemplate}){
  ...
  const FILTERS=[{k:'all',label:'All'},{k:'active',label:'Active'},{k:'monitoring',label:'Monitoring'},{k:'mine',label:'My Files'}];
  return(
    <div style={{display:'flex',height:'100%',overflow:'hidden'}}>
      <div style={{width:w,flexShrink:0,overflowY:'auto',padding:'12px',borderRight:`1px solid ${T.bd}`}}>
        ...
        {['urgent','high'].map(pri=>{const bucket=filtered.filter(f=>f.priority===pri);if(!bucket.length)return null;return<div key={pri} style={{marginBottom:10}}><div style={{fontSize:9,fontWeight:700,color:FP[pri].tx,textTransform:'uppercase',letterSpacing:'0.6px',marginBottom:6,paddingLeft:2}}>{FP[pri].label} ({bucket.length})</div><div style={{display:'flex',flexDirection:'column',gap:4}}>{bucket.map(f=><FileCard key={f.id} file={f} data={data} onClick={()=>setSelFile(selFile===f.id?null:f.id)} selected={selFile===f.id}/>)}</div></div>;})}
        {filtered.filter(f=>!['urgent','high'].includes(f.priority)).length>0&&<div style={{marginBottom:6}}><div style={{fontSize:9,fontWeight:700,color:T.tx3,textTransform:'uppercase',letterSpacing:'0.6px',marginBottom:6,paddingLeft:2}}>Other</div><div style={{display:'flex',flexDirection:'column',gap:4}}>{filtered.filter(f=>!['urgent','high'].includes(f.priority)).map(f=><FileCard key={f.id} file={f} data={data} onClick={()=>setSelFile(selFile===f.id?null:f.id)} selected={selFile===f.id}/>)}</div></div>}
      </div>
...
function FilesView({data,saveFile,saveTask,delTask,newTask,addLogEntry,showAddFile,saveDeliverable,delDeliverable,newDeliverable,applyTemplate}){
  ...
  const [selFile,setSelFile]=useState(null);
  const [splitW,setSplitW]=useState(300);
  const [liveW,setLiveW]=useState(null);
  const w=liveW??splitW;
  const files=data.files.filter(f=>!f.archived).filter(f=>statusF==='all'||f.status===statusF).filter(f=>!search||f.title.toLowerCase().includes(search.toLowerCase())||f.lead?.toLowerCase().includes(search.toLowerCase())).sort((a,b)=>{const po={urgent:0,high:1,medium:2,low:3};return(po[a.priority]||2)-(po[b.priority]||2);});
  const activeFile=selFile?data.files.find(f=>f.id===selFile):null;
  return(
    <div style={{display:'flex',height:'100%',overflow:'hidden'}}>
      <div style={{width:w,flexShrink:0,borderRight:`1px solid ${T.bd}`,display:'flex',flexDirection:'column',overflow:'hidden'}}>
        <div style={{padding:'10px 12px',borderBottom:`1px solid ${T.bd}`,background:T.s1,flexShrink:0}}>
          ...
          <div style={{display:'flex',gap:4,flexWrap:'wrap'}}>{['all',...FILE_STATUS_OPTS].map(s=><button key={s} onClick={()=>setStatusF(s)} style={{...ss.btn,fontSize:10,padding:'2px 8px',background:statusF===s?T.acc:'transparent',color:statusF===s?'#fff':T.tx2,border:`1px solid ${statusF===s?T.acc:T.bd}`}}>{s==='all'?'All':FS[s]?.label||s}</button>)}</div>
        </div>
        <div style={{flex:1,overflowY:'auto'}}>
          {files.map(f=>{const openT=data.tasks.filter(t=>(t.fileId||t.projectId)===f.id&&!isDone(t)).length;const openD=(data.deliverables||[]).filter(d=>d.fileId===f.id&&!isDoneDV(d)).length;return(<div key={f.id} onClick={()=>setSelFile(selFile===f.id?null:f.id)} style={{padding:'10px 12px',borderBottom:`1px solid ${T.bd3}`,cursor:'pointer',background:selFile===f.id?T.s3:'transparent',borderLeft:`3px solid ${selFile===f.id?T.acc:FS[f.status]?.dot||T.tx3}`}} onMouseEnter={e=>{if(selFile!==f.id)e.currentTarget.style.background=T.s2;}} onMouseLeave={e=>{if(selFile!==f.id)e.currentTarget.style.background='transparent';}}><div style={{fontSize:12,fontWeight:600,color:T.tx,marginBottom:3,lineHeight:1.3}}>{f.title}</div><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:2}}><div style={{display:'flex',gap:4,alignItems:'center'}}><StatusDot map={FS} val={f.status}/>{f.priority!=='medium'&&<StatusDot map={FP} val={f.priority}/>}</div><span style={{fontSize:10,color:T.tx3}}>{openT}t{openD>0?` · ${openD}d`:''}</span></div>{f.lead&&<div style={{fontSize:10,color:T.tx3}}>👤 {f.lead}</div>}</div>);})}
        </div>
        <div style={{padding:'8px 12px',borderTop:`1px solid ${T.bd}`,background:T.s1,flexShrink:0}}><button onClick={showAddFile} style={{...ss.btnP,width:'100%',textAlign:'center'}}>+ New file</button></div>
      </div>
      <ResizeHandle currentWidth={w} onResizeLive={setLiveW} onResizeEnd={v=>{setLiveW(null);setSplitW(v);}}/>
...
function TodayView({data,saveTask,delTask,saveUiPref}){
  ...
  // Deliverables due today/overdue
  ...
  const TaskRow=({task,draggable:isDraggable})=>{
    ...
    return(
      <div draggable={isDraggable} onDragStart={()=>setDragId(task.id)} onDragEnd={()=>{setDragId(null);setDragOverId(null);}} onDragOver={e=>{e.preventDefault();setDragOverId(task.id);}} onDrop={()=>handleDrop(task.id)} onClick={()=>setSelTask(selTask===task.id?null:task.id)} style={{display:'flex',alignItems:'center',gap:8,padding:'8px 10px',borderBottom:`1px solid ${T.bd3}`,cursor:'pointer',background:selTask===task.id?T.s3:isDragOver?T.s3:'transparent',borderTop:isDragOver?`2px solid ${T.acc}`:'2px solid transparent',opacity:dragId===task.id?0.4:1}} onMouseEnter={e=>{if(selTask!==task.id&&!isDragOver)e.currentTarget.style.background=T.s2;}} onMouseLeave={e=>{if(selTask!==task.id&&!isDragOver)e.currentTarget.style.background='transparent';}}>
        ...
        <div style={{flex:1,minWidth:0}}>
          ...
          <div style={{display:'flex',gap:4,flexWrap:'wrap',alignItems:'center',marginTop:3}}><StatusDot map={TS} val={task.status}/>{blocked&&<Chip text="⛔ Blocked" bg="rgba(217,95,95,0.12)" tx={T.r} small/>}{task.dueDate&&<DueChip date={task.dueDate}/>}</div>
        </div>
  ...
  const Section=({title,tasks,draggable,accent,extra})=>{if(!tasks.length&&!extra?.length)return null;return<div style={{marginBottom:16}}><div style={{fontSize:9,fontWeight:700,color:accent||T.tx3,textTransform:'uppercase',letterSpacing:'0.7px',marginBottom:4,paddingLeft:2}}>{title} <span style={{opacity:0.6}}>({tasks.length+(extra?.length||0)})</span></div><div style={{border:`1px solid ${T.bd}`,borderRadius:6,overflow:'hidden',background:T.s1}}>{tasks.map(t=><TaskRow key={t.id} task={t} draggable={draggable}/>)}{extra?.map((dv,i)=>(<div key={dv.id} style={{display:'flex',alignItems:'center',gap:8,padding:'8px 10px',borderBottom:`1px solid ${T.bd3}`,background:'rgba(212,146,42,0.04)'}}><div style={{flex:1}}><div style={{fontSize:9,fontWeight:700,color:T.y,textTransform:'uppercase',letterSpacing:'0.4px',marginBottom:1}}>{getFile(data.files,dv.fileId)?.title||''} · Deliverable</div><div style={{fontSize:12,color:T.tx,fontWeight:500}}>{dv.title}</div><div style={{marginTop:3}}><Chip text={dvLabel(dv.type)} bg="rgba(212,146,42,0.10)" tx={T.y} small/>{dv.dueDate&&<span style={{marginLeft:4}}><FlexChip fd={dv.dueDate}/></span>}</div></div></div>))}</div></div>;};

  return(
    <div style={{display:'flex',height:'100%',overflow:'hidden'}}>
      <div style={{flex:1,overflowY:'auto',padding:'14px 16px',maxWidth:660}}>
        <div style={{marginBottom:14}}><h3 style={{margin:'0 0 2px',fontSize:16,fontWeight:700,color:T.tx,fontFamily:T.font}}>{new Date().toLocaleDateString('en-CA',{weekday:'long',month:'long',day:'numeric'})}</h3><div style={{fontSize:11,color:T.tx3}}>{allMyTasks.length} open tasks · {overdue.length} overdue</div></div>
        <Section title="Overdue" tasks={overdue} accent={T.r}/>
        <Section title="Today" tasks={today} draggable accent={T.y} extra={dvDueToday}/>
        {today.length===0&&overdue.length===0&&dvDueToday.length===0&&<div style={{padding:'14px',border:`1px solid ${T.bd}`,borderRadius:6,background:T.s1,marginBottom:14,fontSize:12,color:T.tx3,fontStyle:'italic',textAlign:'center'}}>Nothing due today or overdue.</div>}
...
function CalendarView({data,calMode,setCalMode,saveTask,delTask}){
    ...
    return(
      ...
      <div style={{flex:1,padding:12,display:'flex',flexDirection:'column',overflow:'hidden'}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(7,1fr)',gap:5,flex:1,minHeight:0,overflow:'auto'}}>
          {days.map((day,di)=>{const dStr=toStr(day),isToday=dStr===TODAY_STR;const dayTasks=tasksByDate[dStr]||[];const dayDVs=dvsByDate[dStr]||[];return(<div key={di} style={{background:isToday?'rgba(91,156,246,0.06)':T.s1,border:`1px solid ${isToday?T.acc:T.bd}`,borderRadius:6,padding:'8px',display:'flex',flexDirection:'column'}}><div style={{fontSize:11,fontWeight:700,color:isToday?T.acc:T.tx2,marginBottom:5,flexShrink:0}}>{WD[di]}<br/><span style={{fontSize:14}}>{day.getDate()}</span></div><div style={{flex:1,overflowY:'auto',minHeight:0}}>{dayDVs.map(d=><div key={d.id} style={{background:'rgba(212,146,42,0.10)',border:`1px solid rgba(212,146,42,0.2)`,borderRadius:3,padding:'3px 5px',marginBottom:3,fontSize:9,color:T.y,fontWeight:600}}>{dvLabel(d.type)}: {d.title.slice(0,20)}</div>)}{dayTasks.map(t=>{const file=fileForTask(t);return(<div key={t.id} onClick={()=>setSelTask(selTask===t.id?null:t.id)} style={{background:selTask===t.id?T.s3:T.s2,border:`1px solid ${selTask===t.id?T.acc:T.bd}`,borderRadius:4,padding:'4px 6px',marginBottom:3,cursor:'pointer'}}>{file&&<div style={{fontSize:9,fontWeight:700,color:T.tx3,textTransform:'uppercase',marginBottom:1}}>{file.title?.slice(0,16)}</div>}<div style={{fontSize:11,color:T.tx,lineHeight:1.3}}>{t.title}</div></div>);})}</div></div>);})}
        </div>
      </div>
      {selTask&&<TaskPanel taskId={selTask} data={data} onClose={()=>setSelTask(null)} saveTask={saveTask} delTask={id=>{delTask(id);setSelTask(null);}} onOpenTask={setSelTask}/>}
      </div>
  ...
  return(
    ...
    <div style={{flex:1,padding:12,display:'flex',flexDirection:'column',overflow:'hidden'}}>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(7,1fr)',gap:1,marginBottom:2,flexShrink:0}}>{WD.map(d=><div key={d} style={{fontSize:10,fontWeight:700,color:T.tx3,textAlign:'center',padding:'3px 0'}}>{d}</div>)}</div>
      <div style={{flex:1,display:'flex',flexDirection:'column',gap:1,overflow:'auto'}}>
        {wks.map((wk,wi)=>(
          <div key={wi} style={{display:'grid',gridTemplateColumns:'repeat(7,1fr)',gap:1,flex:1}}>
            {wk.map((day,di)=>{const dStr=toStr(day),isToday=dStr===TODAY_STR,inMonth=day.getMonth()===mo;const dayTasks=(tasksByDate[dStr]||[]).slice(0,2);const dayDVs=(dvsByDate[dStr]||[]).slice(0,1);return(<div key={di} style={{padding:'3px 4px',background:isToday?'rgba(91,156,246,0.06)':inMonth?T.s1:T.s2,border:`1px solid ${isToday?T.acc:T.bd3}`,borderRadius:3,minHeight:52,overflow:'hidden'}}><div style={{fontSize:11,fontWeight:isToday?700:400,color:inMonth?T.tx2:T.tx3,marginBottom:2}}>{day.getDate()}</div>{dayDVs.map(d=><div key={d.id} style={{fontSize:9,background:'rgba(212,146,42,0.12)',color:T.y,borderRadius:2,padding:'1px 3px',marginBottom:1,lineHeight:1.3}}>{d.title.slice(0,14)}</div>)}{dayTasks.map(t=>{const ts=TS[t.status];return(<div key={t.id} onClick={()=>setSelTask(selTask===t.id?null:t.id)} style={{fontSize:10,background:ts?.bg||T.s2,color:ts?.tx||T.tx2,borderRadius:2,padding:'2px 4px',marginBottom:2,cursor:'pointer',lineHeight:1.3,wordBreak:'break-word'}}>{t.title.slice(0,20)}</div>);})}</div>);})}
          </div>
        ))}
      </div>
    </div>
  ...
  return(
    <div style={{display:'flex',height:'100%',overflow:'hidden'}}>
      <div style={{width:240,flexShrink:0,borderRight:`1px solid ${T.bd}`,overflowY:'auto'}}>
        {people.map(p=>{const wl=getWorkload(p.name);return(<div key={p.id} onClick={()=>setSelPerson(selPerson===p.id?null:p.id)} style={{padding:'10px 12px',borderBottom:`1px solid ${T.bd3}`,cursor:'pointer',background:selPerson===p.id?T.s3:'transparent'}} onMouseEnter={e=>{if(selPerson!==p.id)e.currentTarget.style.background=T.s2;}} onMouseLeave={e=>{if(selPerson!==p.id)e.currentTarget.style.background='transparent';}}><div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:2}}><span style={{fontSize:13,fontWeight:600,color:T.tx}}>{p.name}</span>{wl.overdue.length>0&&<span style={{fontSize:10,color:T.r,fontWeight:600}}>{wl.overdue.length} overdue</span>}</div>{p.title&&<div style={{fontSize:11,color:T.tx3,marginBottom:4}}>{p.title}</div>}<div style={{display:'flex',gap:6}}><span style={{fontSize:10,color:T.tx2}}>{wl.filesLed.length} file{wl.filesLed.length!==1?'s':''}</span><span style={{fontSize:10,color:T.tx3}}>·</span><span style={{fontSize:10,color:T.tx2}}>{wl.tasksOpen.length} task{wl.tasksOpen.length!==1?'s':''}</span>{wl.dvOwned.length>0&&<><span style={{fontSize:10,color:T.tx3}}>·</span><span style={{fontSize:10,color:T.acc}}>{wl.dvOwned.length} dv</span></>}</div></div>);})}
      </div>
      <div style={{flex:1,overflowY:'auto',padding:'14px 16px'}}>
        {selPerson?(()=>{const p=people.find(x=>x.id===selPerson);if(!p)return null;const wl=getWorkload(p.name);return(<div><h3 style={{margin:'0 0 4px',fontSize:16,fontWeight:700,color:T.tx,fontFamily:T.font}}>{p.name}</h3>{p.title&&<div style={{fontSize:12,color:T.tx2,marginBottom:14}}>{p.title}</div>}
          {wl.filesLed.length>0&&<div style={{marginBottom:14}}><div style={ss.lbl}>FILES LED ({wl.filesLed.length})</div>{wl.filesLed.map(f=><div key={f.id} style={{display:'flex',alignItems:'center',gap:6,padding:'5px 0',borderBottom:`1px solid ${T.bd3}`}}><span style={{fontSize:12,color:T.tx,flex:1}}>{f.title}</span><StatusDot map={FS} val={f.status}/><StatusDot map={FH} val={f.health}/></div>)}</div>}
          {wl.dvOwned.length>0&&<div style={{marginBottom:14}}><div style={ss.lbl}>DELIVERABLES OWNED ({wl.dvOwned.length})</div>{wl.dvOwned.map(d=>{const file=getFile(data.files,d.fileId);return(<div key={d.id} style={{padding:'5px 0',borderBottom:`1px solid ${T.bd3}`}}>{file&&<div style={{fontSize:9,color:T.tx3,fontWeight:700,textTransform:'uppercase',letterSpacing:'0.3px'}}>{file.title}</div>}<div style={{display:'flex',alignItems:'center',gap:5}}><span style={{fontSize:12,color:T.tx,flex:1}}>{d.title}</span><StatusDot map={DVS} val={d.status}/>{d.dueDate&&<FlexChip fd={d.dueDate}/>}</div></div>);})}</div>}
          {wl.tasksOpen.length>0&&<div style={{marginBottom:14}}><div style={ss.lbl}>OPEN TASKS ({wl.tasksOpen.length})</div>{wl.tasksOpen.map(t=>{const file=getFile(data.files,t.fileId||t.projectId);return(<div key={t.id} style={{padding:'5px 0',borderBottom:`1px solid ${T.bd3}`}}>{file&&<div style={{fontSize:9,color:T.tx3,fontWeight:700,textTransform:'uppercase',letterSpacing:'0.3px'}}>{file.title}</div>}<div style={{display:'flex',alignItems:'center',gap:5}}><span style={{fontSize:12,color:T.tx,flex:1}}>{t.title}</span><StatusDot map={TS} val={t.status}/>{t.dueDate&&<DueChip date={t.dueDate}/>}</div></div>);})}</div>}
        </div>);})():<div style={{display:'flex',alignItems:'center',justifyContent:'center',height:'100%',color:T.tx3,fontSize:13,fontStyle:'italic'}}>Select a person</div>}
      </div>
    </div>
  );
}
...
function TemplatesView({data}){
  ...
  return(
    <div style={{height:'100%',overflow:'auto',padding:'14px 20px',maxWidth:760}}>
      <div style={{marginBottom:14}}>
        <h3 style={{margin:'0 0 2px',fontSize:16,fontWeight:700,color:T.tx,fontFamily:T.serif}}>Templates</h3>
        <p style={{margin:0,fontSize:12,color:T.tx2}}>Select a template to see its tasks. Apply templates from the Deliverables tab inside any file.</p>
      </div>
      <div style={{display:'grid',gridTemplateColumns:'repeat(2,1fr)',gap:8,marginBottom:16}}>
        {BUILT_IN_TEMPLATES.map(t=>(
          <div key={t.id} onClick={()=>setSel(sel===t.id?null:t.id)} style={{padding:'10px 12px',border:`1.5px solid ${sel===t.id?T.acc:T.bd}`,borderRadius:7,cursor:'pointer',background:sel===t.id?T.s3:T.s2}} onMouseEnter={e=>{if(sel!==t.id)e.currentTarget.style.borderColor=T.bd2;}} onMouseLeave={e=>{if(sel!==t.id)e.currentTarget.style.borderColor=T.bd;}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:4}}>
              <div style={{fontSize:12,fontWeight:600,color:T.tx}}>{t.name}</div>
              <span style={{fontSize:9,color:T.tx3,fontFamily:T.mono}}>{t.taskTemplates.length} tasks</span>
            </div>
            <div style={{fontSize:10,color:T.tx3,lineHeight:1.4,marginBottom:4}}>{t.description}</div>
            <div style={{fontSize:9,color:T.acc}}>{dvLabel(t.deliverableType)}</div>
          </div>
        ))}
      </div>
      {tpl&&(
        <div style={{border:`1px solid ${T.bd}`,borderRadius:8,overflow:'hidden'}}>
          <div style={{padding:'10px 14px',background:T.s2,borderBottom:`1px solid ${T.bd}`,display:'flex',justifyContent:'space-between',alignItems:'center'}}>
            <div><div style={{fontSize:13,fontWeight:600,color:T.tx}}>{tpl.name}</div><div style={{fontSize:10,color:T.tx3,marginTop:2}}>{tpl.defaultDurationDays}-day default timeline · apply from a file's Deliverables tab</div></div>
          </div>
          {tpl.taskTemplates.map((tt,i)=>(
            <div key={i} style={{display:'flex',alignItems:'flex-start',gap:10,padding:'10px 14px',borderBottom:`1px solid ${T.bd3}`,background:i%2===0?T.s1:T.s2}}>
              <div style={{width:22,height:22,borderRadius:'50%',background:T.s3,border:`1px solid ${T.bd2}`,display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0,fontSize:10,fontWeight:700,color:T.tx3,marginTop:1}}>{i+1}</div>
              <div style={{flex:1}}>
                <div style={{fontSize:12,fontWeight:500,color:T.tx,marginBottom:2}}>{tt.title}</div>
                {tt.notes&&<div style={{fontSize:11,color:T.tx3,lineHeight:1.4}}>{tt.notes}</div>}
              </div>
              <div style={{flexShrink:0,textAlign:'right'}}>
                {tt.offsetDays!==0?<span style={{fontSize:10,color:T.tx3,fontFamily:T.mono}}>{tt.offsetDays>0?'+':''}{tt.offsetDays}d</span>:<span style={{fontSize:10,color:T.acc,fontFamily:T.mono}}>target</span>}
                {tt.requiresApproval&&<div style={{marginTop:2}}><Chip text="Approval" bg="rgba(212,146,42,0.12)" tx={T.y} small/></div>}
              </div>
            </div>
          ))}
        </div>
...
2. Parse my notes. Identify: completed tasks, new tasks, memory updates, log entries, changed dates, milestone updates, deliverable updates, new SharePoint links, status/health changes.
...
function ClaudeView({data,onImport}){
  ...
  return(
    <div style={{height:'100%',overflow:'auto',padding:'14px 20px',maxWidth:720}}>
      <div style={{marginBottom:14}}><h3 style={{margin:'0 0 2px',fontSize:16,fontWeight:700,color:T.tx,fontFamily:T.serif}}>Claude</h3><p style={{margin:0,fontSize:12,color:T.tx2}}>Export current state for review, then import Claude's structured update package.</p></div>
      ...
      {tab==='export'&&(<div><Fld label="Export scope"><select value={exportScope} onChange={e=>setExportScope(e.target.value)} style={{...ss.sel,width:'auto'}}><option value="active">Active files only</option><option value="mine">My files only</option><option value="all">All files</option></select></Fld><div style={{display:'flex',alignItems:'center',gap:8,marginBottom:12}}><input type="checkbox" id="inclogs" checked={incLogs} onChange={e=>setIncLogs(e.target.checked)}/><label htmlFor="inclogs" style={{fontSize:12,color:T.tx2,cursor:'pointer'}}>Include recent log entries</label></div><button onClick={copyExport} style={{...ss.btnP,marginBottom:10}}>{copied?'✓ Copied!':'Copy export to clipboard'}</button><div style={{fontSize:11,color:T.tx3,lineHeight:1.6}}>Paste this into Claude chat, then paste your meeting notes or week's notes. Claude will compare them and propose structured updates.</div></div>)}
      {tab==='import'&&(<div>{!preview?(<><Fld label="Paste Claude's update package JSON here"><textarea value={importJson} onChange={e=>setImportJson(e.target.value)} rows={12} placeholder='{"importType":"palantir_update_package","version":"1.0",...}' style={{...ss.inp,resize:'vertical',fontFamily:T.mono,fontSize:11}}/></Fld>{importErr&&<div style={{color:T.r,fontSize:11,marginBottom:8}}>{importErr}</div>}<button onClick={()=>parseImport(importJson)} style={ss.btnP}>Validate & Preview</button></>):(<div>{applied?(<div style={{padding:'16px',background:'rgba(63,182,139,0.08)',border:`1px solid rgba(63,182,139,0.2)`,borderRadius:6,textAlign:'center'}}><div style={{fontSize:14,fontWeight:600,color:T.g,marginBottom:4}}>✓ Import applied</div><button onClick={()=>{setPreview(null);setImportJson('');setApplied(false);}} style={ss.btn}>Import another</button></div>):(<><div style={{padding:'12px',background:T.s2,borderRadius:6,marginBottom:12}}><div style={{fontSize:11,fontWeight:600,color:T.tx,marginBottom:4}}>Summary: {preview.summary}</div>{preview.changes.map((c,i)=><div key={i} style={{fontSize:11,color:T.tx2,padding:'2px 0',borderBottom:`1px solid ${T.bd3}`}}>· {c}</div>)}{(preview.imp.warnings||[]).map((w,i)=><div key={i} style={{fontSize:11,color:T.y,marginTop:4}}>⚠ {w}</div>)}</div><div style={{display:'flex',gap:6}}><button onClick={()=>{onImport(preview.imp);setApplied(true);}} style={ss.btnP}>Apply changes</button><button onClick={()=>{setPreview(null);setImportJson('');}} style={ss.btn}>Cancel</button></div></>)}</div>)}</div>)}
      {tab==='help'&&(<div style={{fontSize:12,color:T.tx2,lineHeight:1.8}}><div style={{marginBottom:10,fontWeight:600,color:T.tx}}>Workflow</div>{['1. Go to Export tab and copy the current state.','2. Open a new Claude chat (or your Palantír Claude Project).','3. Paste the export, then paste your weekly notes or meeting transcript.','4. Claude walks you through changes file by file and asks questions.','5. Once confirmed, Claude produces a JSON update package.','6. Come back here, go to Import tab, paste the JSON.','7. Review the preview and click Apply.'].map((s,i)=><div key={i} style={{padding:'5px 0',borderBottom:`1px solid ${T.bd3}`}}>{s}</div>)}<div style={{marginTop:12,padding:'10px',background:T.s2,borderRadius:5,fontSize:11,color:T.tx3}}>The system prompt included in your export tells Claude exactly how Palantír works and what format to use for the import package.</div></div>)}
    </div>
  );
}
...
function AddFileModal({data,onClose,onCreate}){
  ...
  return(<Overlay onClose={onClose}><ModalH title="New File" onClose={onClose}/><Fld label="Title"><Inp value={form.title} onChange={v=>setForm(x=>({...x,title:v}))} placeholder="File name"/></Fld><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}><Fld label="Status" mb={8}><select value={form.status} onChange={e=>setForm(x=>({...x,status:e.target.value}))} style={ss.sel}>{FILE_STATUS_OPTS.map(s=><option key={s} value={s}>{FS[s]?.label||s}</option>)}</select></Fld><Fld label="Priority" mb={8}><select value={form.priority} onChange={e=>setForm(x=>({...x,priority:e.target.value}))} style={ss.sel}>{PRIORITY_OPTS.map(s=><option key={s} value={s}>{FP[s]?.label||s}</option>)}</select></Fld><Fld label="Lead" mb={8}><select value={form.lead} onChange={e=>setForm(x=>({...x,lead:e.target.value}))} style={ss.sel}><option value="">—</option>{allPeople.map(m=><option key={m}>{m}</option>)}</select></Fld><Fld label="Health" mb={8}><select value={form.health} onChange={e=>setForm(x=>({...x,health:e.target.value}))} style={ss.sel}>{HEALTH_OPTS.map(s=><option key={s} value={s}>{FH[s]?.label||s}</option>)}</select></Fld></div><Fld label="Initial memory / context (optional)"><Inp value={form.memory} onChange={v=>setForm(x=>({...x,memory:v}))} placeholder="Brief context…" rows={3}/></Fld><button onClick={()=>{if(form.title.trim()){onCreate(form);onClose();}}} style={{...ss.btnP,width:'100%',marginTop:4}}>Create file</button></Overlay>);
}

function TeamModal({data,onClose,setData}){
  const [newName,setNewName]=useState('');const[newTitle,setNewTitle]=useState('');
  const people=data.people||[];
  return(<Overlay onClose={onClose} wide><ModalH title="People & Team" onClose={onClose}/><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:20}}><div><div style={ss.lbl}>TEAM MEMBERS</div><div style={{marginBottom:10}}>{people.map(p=>(<div key={p.id} style={{display:'flex',alignItems:'center',padding:'6px 0',borderBottom:`1px solid ${T.bd3}`}}><div style={{flex:1}}><div style={{fontSize:13,color:T.tx,fontWeight:500}}>{p.name}</div>{p.title&&<div style={{fontSize:10,color:T.tx3}}>{p.title}</div>}</div>{p.name!=='Karl'&&<button onClick={()=>setData(d=>({...d,people:d.people.map(x=>x.id===p.id?{...x,active:!x.active}:x)}))} style={{...ss.btn,fontSize:10,padding:'2px 7px',color:p.active!==false?T.tx2:T.r}}>{p.active!==false?'Active':'Inactive'}</button>}</div>))}</div><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:4,marginBottom:4}}><Inp value={newName} onChange={setNewName} placeholder="Name"/><Inp value={newTitle} onChange={setNewTitle} placeholder="Title / Role"/></div><button onClick={()=>{if(newName.trim()&&!people.find(p=>p.name===newName.trim())){setData(d=>({...d,people:[...(d.people||[]),{id:uid(),name:newName.trim(),title:newTitle.trim(),active:true}]}));setNewName('');setNewTitle('');}}} style={ss.btnP}>Add person</button></div><div><div style={ss.lbl}>QUICK WORKLOAD SUMMARY</div>{people.filter(p=>p.active!==false).map(p=>{const led=data.files.filter(f=>f.lead===p.name&&!f.archived).length;const tasks=data.tasks.filter(t=>taskAssignees(t).includes(p.name)&&!isDone(t)).length;const overdue=data.tasks.filter(t=>taskAssignees(t).includes(p.name)&&!isDone(t)&&t.dueDate&&ds(t.dueDate)==='overdue').length;const dvs=(data.deliverables||[]).filter(d=>d.ownerName===p.name&&!isDoneDV(d)).length;return(<div key={p.id} style={{padding:'6px 0',borderBottom:`1px solid ${T.bd3}`}}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}><span style={{fontSize:12,color:T.tx,fontWeight:500}}>{p.name}</span><div style={{display:'flex',gap:6}}><span style={{fontSize:10,color:T.tx2}}>{led}f</span><span style={{fontSize:10,color:T.tx2}}>{tasks}t</span>{dvs>0&&<span style={{fontSize:10,color:T.acc}}>{dvs}dv</span>}{overdue>0&&<span style={{fontSize:10,color:T.r}}>{overdue} overdue</span>}</div></div></div>);})}</div></div></Overlay>);
}
...
export default function App(){
  ...
  useEffect(()=>{
    (async()=>{
      try{const{data:row}=await supabase.from('palantir_state').select('state').eq('id',1).maybeSingle();if(row?.state&&Object.keys(row.state).length>0){setData({deliverables:[],...row.state});return;}const{data:oldRow}=await supabase.from('planner_state').select('state').eq('id',1).maybeSingle();if(oldRow?.state&&Object.keys(oldRow.state).length>0){setData(migrateFromPlanner(oldRow.state));}else{setData({files:[],tasks:[],deliverables:[],people:[{id:'per-1',name:'Karl',title:'',active:true}],templates:[],uiPrefs:{},version:'1.0'});}}catch(e){console.error('Load error',e);setData({files:[],tasks:[],deliverables:[],people:[{id:'per-1',name:'Karl',title:'',active:true}],templates:[],uiPrefs:{},version:'1.0'});}
    })();
  },[]);
    ...
    <div style={{fontFamily:T.font,height:'100vh',display:'flex',flexDirection:'column',background:T.bg,overflow:'hidden',color:T.tx}}>
      ...
      <div style={{background:T.hdr,borderBottom:`1px solid ${T.bd}`,padding:'0 14px',display:'flex',alignItems:'center',height:44,flexShrink:0,gap:0}}>
        ...
        <div style={{display:'flex',gap:0}}>
          {NAV.map(n=><button key={n.id} onClick={()=>setView(n.id)} style={{padding:'4px 12px',fontSize:12,fontWeight:500,border:'none',background:'transparent',cursor:'pointer',color:view===n.id?T.acc:T.tx2,borderBottom:`2px solid ${view===n.id?T.acc:'transparent'}`,borderRadius:0,fontFamily:T.font,transition:'color .1s'}}>{n.label}</button>)}
        </div>
        {view==='calendar'&&<div style={{display:'flex',gap:3,marginLeft:10}}>{['monthly','weekly'].map(m=><button key={m} onClick={()=>setCalMode(m)} style={{...ss.btn,fontSize:10,background:calMode===m?T.acc:'transparent',color:calMode===m?'#fff':T.tx2,border:`1px solid ${calMode===m?T.acc:T.bd}`,textTransform:'capitalize'}}>{m}</button>)}</div>}
      ...
      <div style={{flex:1,overflow:'hidden',display:'flex',flexDirection:'column'}}>
        ...
        {view==='templates' &&<TemplatesView data={data}/>}
